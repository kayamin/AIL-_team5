...
class RepDetector:
    ...
    def do_local_count(self, initial):
        ...
        # framesArr = Variable(torch.FloatTensor(framesArr).cuda())
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            # ------------------------------------- Request Openpose to estimate menu -------------------------------------
            if cur_state in (state.IN_REP, state.COOLDOWN):
                if start_frame < 0:
                    start_frame = frame_counter
                if (frame_counter - start_frame) % openpose_interval_frame == 0:
                    # img = cv2.putText(img, "%03d" % frame_count, (0, height), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255))
                    cv2.imwrite("/home/team05/Downloads/openpose_zmq/hub.png", frame)
                    with open("/home/team05/Downloads/openpose_zmq/new_image.txt", "w") as f:
                        print("1", file=f)
                
                # ------------------------------------- See if Reply has come -------------------------------------
                with open("/home/team05/Downloads/openpose_zmq/estimated.txt", "r") as f:
                    s = f.readline().strip()
                    if s == "1":
                        ...
                        if result < 0:
                            ...
                            print("{0:03d}: {1}".format(frame_counter, menu))
                        # frame = cv2.putText(frame, menu, (50, 100), cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 5)
                        with open("/home/team05/Downloads/openpose_zmq/estimated.txt", "w") as f:
                            print("0\n-1", file=f)

                draw_str(frame, (20, 240), menu, red_color, 2)
            else:
                start_frame = -1
