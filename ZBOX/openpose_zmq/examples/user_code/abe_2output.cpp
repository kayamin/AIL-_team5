...
DEFINE_string(output_resolution,        "-1x480",        "The image resolution (display and output). Use \"-1x-1\" to force the program to use the"
...
class UserPredictClass : public UserOutputClass
{
public:
    svm_model* model;
    ...
    UserPredictClass(std::string svm_model_file){
        if ((this->model = svm_load_model(svm_model_file.c_str())) == NULL){
            op::error("Couldn't load svm_model: " + svm_model_file, __LINE__, __FUNCTION__, __FILE__);
        }
    }

    bool chooseLeftOrRight(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr,
                                        std::vector<double>& x_coord, 
                                        std::vector<double>& y_coord, 
                                        int person = 0  // Look at person 0
                                        ){
        if (x_coord.size() != 3){
            op::error("x_coord doesn't have size of 3 (from chooseLeftOrRight):" + std::to_string(x_coord.size()), __LINE__, __FUNCTION__, __FILE__);
            return false;
        }
        if (y_coord.size() != 3){
            op::error("y_coord doesn't have size of 3 (from chooseLeftOrRight):" + std::to_string(y_coord.size()), __LINE__, __FUNCTION__, __FILE__);
            return false;
        }
        const auto& poseKeypoints = datumsPtr->at(0).poseKeypoints;
        ...
        std::vector<double> full_x_coord;
        std::vector<double> full_y_coord;
        std::vector<double> full_conf;

        std::vector<int> rightArmIdx{2, 3, 4};
        std::vector<int> leftArmIdx{5, 6, 7};

        double rightConfSum = 0, leftConfSum = 0;
        bool rightHasZeroConf = false, leftHasZeroConf = false;
        double x, y, conf;
        double x_max=0, x_min=1e6, y_max=0, y_min=1e6;
    
        // Extract full data from datumsPtr
        for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++){
            ...
            if (x_max < x){x_max = x;}
            if (x_min > x && conf > 0.05){x_min = x;}
            if (y_max < y){y_max = y;}
            if (y_min > y && conf > 0.05){y_min = y;}
        ...
        // op::log("full_x_coord.size():" + std::to_string(full_x_coord.size()));
        // op::log("full_y_coord.size():" + std::to_string(full_y_coord.size()));
        // op::log("full_conf.size():" + std::to_string(full_conf.size()));

        // get confidences (if there is 0, set ~HasZeroConf to true)
        for (auto j: rightArmIdx){
            if (full_conf[j] == 0){rightHasZeroConf = true;}
            rightConfSum += full_conf[j];
        }
        for (auto j: leftArmIdx){
            if (full_conf[j] == 0){leftHasZeroConf = true;}
            leftConfSum += full_conf[j];
        }

        // validity
        if (rightHasZeroConf && leftHasZeroConf){
            return false;
        }
        else{
            int j;
            if (leftHasZeroConf){
                for (auto i = 0; i < (int)rightArmIdx.size(); i++){
                    j = rightArmIdx[i];
                    x_coord[i] = (full_x_coord[j] - x_min)/(x_max - x_min);
                    y_coord[i] = (full_y_coord[j] - y_min)/(y_max - y_min);
                }
            }
            else if (rightHasZeroConf){
                for (auto i = 0; i < (int)leftArmIdx.size(); i++){
                    j = leftArmIdx[i];
                    x_coord[i] = (full_x_coord[j] - x_min)/(x_max - x_min);
                    y_coord[i] = (full_y_coord[j] - y_min)/(y_max - y_min);
                }
    ...
    bool calcFeature(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr, std::vector<double>& feature){
        // Extract Right Arm & Left Arm
        std::vector<double> x_coord(3);
        std::vector<double> y_coord(3);
        
        if (chooseLeftOrRight(datumsPtr, x_coord, y_coord)){
            if (x_coord.size() != y_coord.size()){
                op::error("x_coord & y_coord must have same size: " + 
                std::to_string((int)x_coord.size()) + ", " + std::to_string((int)x_coord.size()), __LINE__, __FUNCTION__, __FILE__);
                return false;
            }
            if (!feature.empty()){
                op::error("feature must be empty (calcFeature)", __LINE__, __FUNCTION__, __FILE__);
                return false;
            }
            double dx, dy;
            for (auto i = 0; i < (int)x_coord.size(); i++){
                for (auto j = i + 1; j < (int)x_coord.size(); j++){
                    dx = x_coord[i] - x_coord[j];
                    dy = y_coord[i] - y_coord[j];
                    feature.push_back(dx*dx + dy*dy);
                }
            }
            return true;
        }
    ...
    void featureToNode(std::vector<double>& feature, std::vector<svm_node>& node){
  ...
  void outputFeature(std::vector<double>& feature, std::string file){
    ...
    void predictUpDown(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
        // Example: How to use the pose keypoints
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            std::vector<double> feature;
            std::vector<svm_node> svm_input;
            if (!calcFeature(datumsPtr, feature)){
                op::log("UNKNOWN");
            }
            else{
	      //outputFeature(feature, "./output_feature.txt");
                featureToNode(feature, svm_input);
                const auto kPredictedLabel = svm_predict(this->model, &svm_input[0]);
                op::log(kPredictedLabel == 0 ? "UP" : "DOWN");
            }
        }
        else
            op::log("Nullptr or empty datumsPtr found.", op::Priority::High, __LINE__, __FUNCTION__, __FILE__);
  ...
  void outputPose(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr){
    if (datumsPtr != nullptr && !datumsPtr->empty()){
        const auto& poseKeypoints = datumsPtr->at(0).poseKeypoints;
        if (poseKeypoints.empty()){
            op::log("***NO PERSON IN VISION***");
	    ...
	    double x, y, conf;
            for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++){
                ...
                if (x_max < x){x_max = x;}
                if (x_min > x && conf > 0){x_min = x;}
                if (y_max < y){y_max = y;}
                if (y_min > y && conf > 0){y_min = y;}
                ofs << x << " " << y << " " << conf << " ";
            }
            ofs << x_max << " " << x_min << " " << y_max << " " << y_min;
            ofs << std::endl;
            ofs.close();
        }
    }
...
int openPoseTutorialWrapper1()
{
    ...
    // outputSize
    const auto outputSize = op::flagsToPoint(FLAGS_output_resolution, "-1x480");
    // netInputSize
    ...
    UserPredictClass userOutputClass("./train2.txt.model");
    bool userWantsToExit = false;
    int frame = 0;
    while (!userWantsToExit)
    {
      op::log("frame: " + std::to_string(frame));
      frame++;
      // Pop frame
	   ...
	   userWantsToExit = userOutputClass.display(datumProcessed);
            ...
            userOutputClass.predictUpDown(datumProcessed);
	    // userOutputClass.outputPose(datumProcessed);
        }
        else
