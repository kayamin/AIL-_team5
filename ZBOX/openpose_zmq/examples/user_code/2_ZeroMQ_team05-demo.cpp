...
int openPoseTutorialPose1()
{
  //op::log("OpenPose Library Tutorial - Example 1.", op::Priority::High);
    ...
    // Step 5 (ADDITIONAL) - Initialize ZeroMQ socket
    std::cout << "Setting Up ZeroMQ" << std::endl;
    ...
    std::cout << "Set Up ZeroMQ" << std::endl;
    cv::waitKey(3000);

    //while (1){
        // ------------------------- ZMQ MESSAGE RECEPTION -------------------------
      std::cout << "Start receving" << std::endl;
        // receive frame_count 
        socket.recv(&rcv_msg, 0);
	std::cout << "Receving frame_count" << std::endl;
        frame_count = *(int*)rcv_msg.data();
	std::cout << "Received Frame_count" << std::endl;
        // receive height
        socket.recv(&rcv_msg, 0);
        rows = *(int*)rcv_msg.data();
	std::cout << "Received Height" << std::endl;
        // receive width
        socket.recv(&rcv_msg, 0);
        cols = *(int*)rcv_msg.data();
	std::cout << "Received Width" << std::endl;
        // receive ch
        socket.recv(&rcv_msg, 0);
	...
	if (prev_frame == frame_count){
	  //	  continue;
        ...
        if (type == 2) {
            inputImage = cv::Mat(rows, cols, CV_8UC1, data);
        }
        else {
            inputImage = cv::Mat(rows, cols, CV_8UC3, data);
        }
        ...
        // cv::imwrite(FLAGS_output_image, outputImage);
        // Step 2 - Logging information message
        //op::log("Example 1 successfully finished.", op::Priority::High);

        // Step 3 - print poseKeypoints by hatayama
