...
def load_data( bpath, subjects, actions, dim=3 ):
  ...
  Returns:
    data: Dictionary with keys k=(subject, action, seqname)
      values v=(nx(32*2) matrix of 2d ground truth)　＊3次元のデータを読み込んだ際には  v=nx(32*3) となっているので注意
      There will be 2 entries per subject/action if loading 3d data 各アクションにつき２つデータが有る
      There will be 8 entries per subject/action if loading 2d data 今回の著者が用意したデータには2Dについてのデータは含まれていない
  """
  ...
  for subj in subjects:
    for action in actions:
      ...
      # ３次元の動きを保存したデータのパスを指定（同じ個人の同じ行動でも，複数あるのでAction*.h5 で指定 (sitting1.h5, sitting.h5))
      dpath = os.path.join( bpath, 'S{0}'.format(subj), 'MyPoses/{0}D_positions'.format(dim), '{0}*.h5'.format(action) )
      print( dpath )

      # ファイル名に * が入っているので，それを用いて，条件に合うパスを取得
      fnames = glob.glob( dpath )

      loaded_seqs = 0
      for fname in fnames:
        seqname = os.path.basename( fname ) # パスからファイル名を取得, 特定のファイル名であった場合の条件分岐に使用
...
def load_stacked_hourglass(data_dir, subjects, actions):
  ...
  Returns
    data: dictionary with keys k=(subject, action, seqname)
          values v=(nx(32*2) matrix of 2d stacked hourglass detections)　データを見てみると nx16*2 ?
...
def transform_world_to_camera(poses_set, cams, ncams=4 ):
    ...
    for t3dk in sorted( poses_set.keys() ):
      ...
      for c in range( ncams ):
        ...
        camera_coord = cameras.world_to_camera_frame( np.reshape(t3d_world, [-1, 3]), R, T) #n x (pose*3) -> (n*pose)x3
        camera_coord = np.reshape( camera_coord, [-1, len(H36M_NAMES)*3] ) # (n*pose)x3 -> n x (pose*3)
...
def normalize_data(data, data_mean, data_std, dim_to_use ):
  ...
  for key in data.keys():
    ...
    data_out[ key ] = np.divide( (data[key] - mu), stddev ) # 行毎に各要素毎に割算（ 標準化　）
...
def read_3d_data( actions, data_dir, camera_frame, rcams, predict_14=False ):
  ...
  """
  # Load 3d data
  # trainset[(subject, action, seqname（ファイル名))] = n x (32*2) のデータとして返ってくる
  train_set = load_data( data_dir, TRAIN_SUBJECTS, actions, dim=3 )
  test_set  = load_data( data_dir, TEST_SUBJECTS,  actions, dim=3 )

  # 世界座標（全カメラで取ったデータに共通する座標) から カメラの座標に変換
  if camera_frame:
...
def postprocess_3d( poses_set ):
  ...
  for k in poses_set.keys():
    ...
    poses = poses_set[k]
    poses = poses - np.tile( poses[:,:3], [1, len(H36M_NAMES)] ) # データは n x (3x32) で一行に (Hip3次元)(RHip3次元)・・・
                                                                 # という順番で入っている．最初の Hip の分を全部の姿勢分コピーして
                                                                 # 差し引く (n 次元の方向については BroadCastで対応)
    poses_set[k] = poses
