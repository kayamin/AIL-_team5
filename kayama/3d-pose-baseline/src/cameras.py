...
def project_point_radial( P, R, T, f, c, k, p ):
  ...
  XX = X[:2,:] / X[2,:]
  r2 = XX[0,:]**2 + XX[1,:]**2 # スライスによって，１次元の部分は無くなる (2,np) -> (np,)

  # 放射歪の修正に用いる値
  radial = 1 + np.einsum( 'ij,ij->j', np.tile(k,(1, N)), np.array([r2, r2**2, r2**3]) );
                                                        # k : 3x1 -> 3xnpに, r2は [(np,), (np,), (np,)] で (3,np) となる
  tan = p[0]*XX[1,:] + p[1]*XX[0,:]

  # やりたいことは分かるが，どう実装されているのかが不明
...
def load_camera_params( hf, path ):
  ...
  T = hf[ path.format('T') ][:] # example 'subject1/camera2T'
  f = hf[ path.format('f') ][:]
  c = hf[ path.format('c') ][:]
  k = hf[ path.format('k') ][:]
  p = hf[ path.format('p') ][:]
